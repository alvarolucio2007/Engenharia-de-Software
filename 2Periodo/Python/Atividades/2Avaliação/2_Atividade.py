...
def exercise_3(movie):
    ...
    movie_director = input("Who's the director of the movie?")
...
def exercise_5(list_of_dicts):
    return [
        list_of_dicts[d]["Name"]
        for d in range(len(list_of_dicts))
        if list_of_dicts[d]["Score"] >= 7
    ]
...
def exercise_7(work_friends, uni_friends):
    ...
    return [
        (work_friends_set | uni_friends_set),
        (work_friends_set & uni_friends_set),
        (work_friends_set - uni_friends_set),
    ]
    ...
    while True:
        ...
        try:
            while True:
                choice = int(
                    input(
                        "Please choose whether you want to check or add something to the dictionary! (1=check, 2=add)"
                    )
                )
                if choice in [1, 2]:
                    break
        ...
        if choice == 1:
            while True:
                try:
                    product = input(
                        "Which product would you like to check? (type exit to exit)"
                    )
            ...
            while True:
                product = input(
                    "Which product would you like to add? (type exit to exit)"
                )
                if product.lower() == "exit":
                    break
                while True:
...
print(
    exercicio_13(
        ["200.135.80.9", "192.168.1.1", "200.135.80.9", "192.168.1.1", "202.45.12.3"]
    )
)
                ...
                for i in range(len(list_)):
                    for key, value in list_[i]:
                        if user_book_search in key:
                            print(
                                f"The book {user_book_search} is in the list, with {value} copies!"
                            )
                        else:
                            print(f"The book {user_book_search} is not on the list.")
                    user_book_answer = input(
                        "Would you like to continue to search for another book? (y/n)"
                    )
